...
namespace DevKit.Domain.ValueObjects;

public class Email : ValueObject
{
    private static readonly string _addressPattern =
        @"(?<address>(?<username>[\w\.\-,]+)@(?<domain>([\w\-\.]+)*[\w+]))";

    private static readonly Regex _addressOnlyPattern = new($@"^{_addressPattern}$");

    private static readonly Regex _fullEmailPattern =
        new($@"^""(?<display_name>[\w\s\.\-,@]+)""\s+<{_addressPattern}>$");

    private Email() => Address = Username = Domain = string.Empty;
    ...
    private Email(string address, string displayName, string username, string domain) : this(address,
        username, domain) => DisplayName = displayName;
    ...
    public static Email Parse(string word) {
        ...
        if (match.Success)
            return new(match.Groups["address"].Value, match.Groups["display_name"].Value,
                match.Groups["username"].Value, match.Groups["domain"].Value);

        match = _addressOnlyPattern.Match(word);
        if (match.Success)
            return new(match.Groups["address"].Value, match.Groups["username"].Value,
    ...
    public static Email Create(string displayName, string address) {
        ...
        return new(address, displayName, match.Groups["username"].Value, match.Groups["domain"].Value);
    }
