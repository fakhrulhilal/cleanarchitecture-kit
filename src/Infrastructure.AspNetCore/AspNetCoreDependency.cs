...
namespace Microsoft.Extensions.DependencyInjection
{
    ...
    {
        public static IServiceCollection AddAspNetCore(this IServiceCollection services, string baseApiPath = "/api") => services
            ...
            .AddProblemDetails(opt =>
            {
                ...
                {
                    typeof(ValidationException), typeof(NotFoundException), typeof(UnauthenticatedException),
                    typeof(ForbiddenAccessException)
                };
                opt.Rethrow<StackOverflowException>();
                opt.Rethrow<OutOfMemoryException>();
                opt.Map<ValidationException>(validation => new ValidationProblemDetails(validation.Errors)
                {
                    Title = "Bad request",
                    Status = StatusCodes.Status400BadRequest
                });
                opt.Map<NotFoundException>(_ => new StatusCodeProblemDetails(StatusCodes.Status404NotFound)
                {
