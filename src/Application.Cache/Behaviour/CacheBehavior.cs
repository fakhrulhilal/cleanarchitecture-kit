...
namespace FM.Application.Cache.Behaviour
{
    ...
    /// When injected into a MediatR pipeline, this behavior will receive a list of instances of <see cref="ICacheRegistrar{TRequest,TResponse}"/>
    ...
    /// response cached using the logic and settings in the derived implementation of <see cref="ICacheRegistrar{TRequest,TResponse}"/>.
    /// </summary>
        ...
        where TRequest : IRequest<TResponse>
    {
        ...
        public CacheBehavior(ILogger<TRequest> logger, IEnumerable<ICacheRegistrar<TRequest, TResponse>> cachedRequests)
        {
            _logger = logger;
