using static DevKit.Application.Validations.DomainAddressValidator;
...
public class GivenDomainAddressValidator
{
    ...
    [Test]
    public void WhenContainsDashOrUnderscoreThenItIsValid() =>
        Assert.That(IsValid("another-domain_name"), Is.True);

    [Test]
    public void WhenStartedByDashCharacterThenItIsNotValid() =>
        Assert.That(IsValid("-domain.name"), Is.False);

    [Test]
    public void WhenSomePartStartedByDashThenItIsNotValid() =>
        Assert.That(IsValid("www.-domain.com"), Is.False);

    [Test]
    public void WhenSomePartEndedByDashThenItIsNotValid() =>
        Assert.That(IsValid("www.domain-.com"), Is.False);
