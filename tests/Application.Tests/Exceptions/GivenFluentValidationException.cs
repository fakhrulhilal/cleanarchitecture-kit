...
namespace DevKit.Application.Tests.Exceptions;
...
public class GivenFluentValidationException
{
    ...
    public void WhenContainingMultipleValidationFailureThenItWillBeGroupedByPropertyName() {
        ...
        Assert.That(sut["Age"], Is.EquivalentTo(new[] { "must be 25 or younger", "must be 18 or older" }));
        Assert.That(sut["Password"],
            Is.EquivalentTo(new[] {
