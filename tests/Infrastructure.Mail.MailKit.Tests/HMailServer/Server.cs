...
namespace DevKit.Infrastructure.Mail.MailKit.Tests.HMailServer;
...
internal class Server
{
    ...
    private hMailServer.Application _mailServer = Mock.Of<hMailServer.Application>();

    public Server(IConfiguration config) =>
        _config = config ?? throw new ArgumentNullException(nameof(config));

    internal void Connect() {
        var serverConfig = _config.GetSection("Email:Server").Get<Config>() ?? new();
        try {
    ...
    private void PopulateConnections() {
        ...
        for (int i = 0; i < _mailServer.Settings.TCPIPPorts.Count; i++) {
            ...
            MailConnectionConfig config = protocol switch {
                MailProtocol.Smtp => new EmailConfig.OutgoingConfig {
                    Port = portSetting.PortNumber,
                    UseSecureMode = portSetting.UseSSL,
                    Protocol = MailProtocol.Pop3
                },
                MailProtocol.Pop3 => new EmailConfig.IncomingConfig {
                    Port = portSetting.PortNumber,
                    UseSecureMode = portSetting.UseSSL,
                    Protocol = MailProtocol.Pop3
                },
                MailProtocol.Imap => new EmailConfig.IncomingConfig {
                    Port = portSetting.PortNumber,
                    UseSecureMode = portSetting.UseSSL,
...
#pragma warning disable CA1416 // The test is intended only for Windows OS
